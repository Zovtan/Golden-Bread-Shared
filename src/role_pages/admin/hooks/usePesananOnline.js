...
import { todayWIB, asUTC, fmtDateTime, toWIBDate } from "../../../shared/utils/format";
...
export function usePesananOnline(search = "") {
  ...
  useEffect(() => {
    ...
    then(({ data }) => {
      if (data) setBatas({
        ...data,
        jam_mulai_segera: String(data.jam_mulai_segera ?? "08:00").slice(0, 5),
        jam_selesai_segera: String(data.jam_selesai_segera ?? "17:00").slice(0, 5),
        jam_mulai_preorder: String(data.jam_mulai_preorder ?? "08:00").slice(0, 5),
        jam_selesai_preorder: String(data.jam_selesai_preorder ?? "17:00").slice(0, 5),
        waktu_antar_preorder: (() => {try {return JSON.parse(data.waktu_antar_preorder);} catch {return DEFAULT_BATAS.waktu_antar_preorder;}})(),
        menerima_segera: data.menerima_segera ?? true,
        menerima_preorder: data.menerima_preorder ?? true,
        menit_highlight_segera: data.menit_highlight_segera ?? DEFAULT_BATAS.menit_highlight_segera
      });
    });
  }, []);
  ...
  useRealtimeRefresh("pesanan-online", WATCHED, useCallback(() => fetchPesanan(true), [fetchPesanan]));
  ...
  const filteredList = pesananList.filter((p) => {
    ...
    if (search.trim()) {
      ...
      const itemStr = (p.detail_pesanan_online ?? []).map((d) => d.produk?.nama_produk ?? "").join(" ").toLowerCase();
      if (!(p.pelanggan?.nama_lengkap ?? "").toLowerCase().includes(q) &&
      !(p.pelanggan?.nama_toko ?? "").toLowerCase().includes(q) &&
      !(p.jenis_pembayaran ?? "").toLowerCase().includes(q) &&
      !(p.status ?? "").toLowerCase().includes(q) &&
      !itemStr.includes(q) &&
      !String(p.id_pesanan).includes(q)) return false;
    }
    return true;
  });
        ...
        sebelum === "Pending" || sebelum === "Diproses")) {
          try {
            await refundMidtrans({
              ...
              reason: pesanPembatalan || "Dibatalkan oleh admin"
            });
            didRefund = true;
          } catch (refundErr) {
  ...
  async function editBatas(newBatas) {
    const { error } = await supabase.from("konfigurasi_pesanan").update(newBatas).eq("id", 1);
    if (error) throw error;
    setBatas((prev) => ({
      ...prev, ...newBatas,
      waktu_antar_preorder: (() => {
        if (Array.isArray(newBatas.waktu_antar_preorder)) return newBatas.waktu_antar_preorder;
        try {return JSON.parse(newBatas.waktu_antar_preorder);} catch {return prev.waktu_antar_preorder;}
      })(),
      menit_highlight_segera: newBatas.menit_highlight_segera ?? prev.menit_highlight_segera
    }));
  }

  return {
    pesananList: filteredList,
    stats, batas, loading, error, lastUpdated,
    ...
    updateStatus, rejectRefund, editBatas
  };
}