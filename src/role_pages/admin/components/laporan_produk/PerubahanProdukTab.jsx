...
import { usePerubahanProduk } from "../../hooks/useLaporanProduk";
...
const FIELD_OPTS = ["Semua", "Stok", "Harga Satuan", "Status", "Nama Produk", "Kategori", "Stok Minimal", "Est. Kadaluarsa (hari)", "Deskripsi", "Resep", "Produk Baru"];


export default function PerubahanProdukTab({ search = "" }) {
  const { rows, userList, loading, error, fetch, fetchRef } = usePerubahanProduk();
  ...
  return (
    <div>
      <div className="laporan-filter">
        <div style={{ fontWeight: 600, fontSize: ".875rem", marginBottom: ".75rem" }}>Filter Perubahan Produk:</div>
        ...
        <table className="db-table" style={{ minWidth: 820 }}>
          <thead>
            <tr>
              ...
              <SortableTh label="Produk" colKey="produk" type="string" sortKey={sortKey} sortDir={sortDir} onSort={toggleSort} />
          ...
          <tbody>
            ...
            {rowsSortedPaged.map((r, i) => {
              ...
              const isSub = i > 0 && prevRow?.produk === r.produk && prevRow?.id_ref === r.id_ref;
              return (
                <tr key={r.id}>
                  <td style={{ whiteSpace: "nowrap" }}>{r.tanggal}</td>
                  <td style={{ fontWeight: isSub ? 400 : 500, color: isSub ? "#9ca3af" : undefined }}>
                    {isSub ? <span style={{ marginRight: 4, color: "#9ca3af" }}>↳</span> : null}{r.produk}
                  </td>
