...
import { usePesananOnline } from "../hooks/usePesananOnline";
import DetailPesananModal from "../components/pesanan_online/DetailPesananModal";
import EditBatasPesananModal from "../components/pesanan_online/EditBatasPesananModal";
import KonfirmasiStatusModal from "../components/pesanan_online/KonfirmasiStatusModal";
import BatalkanModal from "../components/pesanan_online/BatalkanModal";
import RejectRefundModal from "../components/pesanan_online/RejectRefundModal";
import ActionErrorModal from "../components/pesanan_online/ActionErrorModal";
...
export default function PesananOnlinePage({ search = "" }) {
  const {
    pesananList, stats, batas, loading, error, lastUpdated,
  ...
  const handleBatalkan = (pesanan) => {
    setBatalModal(pesanan);
  };
    ...
    <div>
      ...
      <div className="db-card" style={{ padding: "1rem 1.25rem", marginBottom: 16 }}>
        <div style={{ display: "flex", alignItems: "flex-end", gap: 16, flexWrap: "wrap" }}>
          ...
          <button className="btn-primary" style={{ alignSelf: "flex-end" }} onClick={() => openModal("batas")}>
            Edit Konfigurasi
          </button>
        </div>
      </div>
                  ...
                  <td style={{ whiteSpace: "nowrap" }}>
                    {p.status === "Pending" && p.refund_status === "Diminta" &&
                      <>
                        ...
                        onClick={() => handleBatalkan(p)}>Setujui Refund</button>
                    ...
                    {p.status === "Pending" && p.refund_status !== "Diminta" &&
                      <>
                        ...
                        <span style={{ color: "#d1d5db", margin: "0 4px" }}>/</span>
                        <button className="db-action-link" style={{ color: "#ef4444" }} disabled={isUpdating}
                        onClick={() => handleBatalkan(p)}>Batalkan</button>
                      </>
                      }
                    {p.status === "Diproses" &&
                      <>
      ...
      <EditBatasPesananModal open={modal === "batas"} onClose={closeModal} batas={batas} onSubmit={editBatas} />
...
export function PesananSearchBar({ value, onChange }) {
  return (
